...
zip объект представляет собой генератор-итератор, а значит мы его можем превратить в другую коллекцию
однако пройти данную последовательность нельзя более 1-го раза. Также это относится к map() filter()
...
Функции map(), filter() тоже представляют собой функции и возвращают свои объекты map и filter соответственно
первым аргументом они оба принимают функцию, а вторым аргументом итерабильную последовательность, только вот в
filter() функция должна возвращать True False и вместо функции может принимать объект данных None, на выходе данные
функции возвращают свои объекты, представляющие собой итераторы, значит можно их переделать в другую коллекцию
функции могут быть разные, анонимные, собственные, встроенные, функции объектов данных и т.д.
...
b = filter(is_simple, a) # is_simple is a link, not func, only link for func
print(tuple(b))
